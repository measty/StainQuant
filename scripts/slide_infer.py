...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="ckpt_dir", help="Config file. ckpt_dir will look for config.yaml in the checkpoint directory")
    parser.add_argument(
        "--output_csv", default="stain_quant.csv", help="Where to save results"
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=None,
        help="Ignore stain values below this threshold",
    )
    ...
    main(
        wsi_path=args.wsi,
        ckpt_path=args.ckpt,
        config_path=args.config if args.config != "ckpt_dir" else Path(args.ckpt).parent / "config.yaml",
